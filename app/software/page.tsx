...
import { Search, ChevronLeft, ChevronRight, Download, Sparkles, Filter } from 'lucide-react'
...
export default async function SoftwarePage({ searchParams }: { searchParams: Promise<SearchParams> }) {
  ...
  try {
    ...
    errorMsg = 'Could not retrieve software files from database.'
  }
      ...
      <div className="grid grid-cols-1 lg:grid-cols-4 gap-8 items-start">
        ...
        <aside className="lg:col-span-1 space-y-6">
          ...
          <div className="bg-gray-50 border border-gray-200 p-5 space-y-3">
            ...
            <form action="/software" method="GET" className="relative">
              <input
                type="text"
                name="search"
                defaultValue={search}
                placeholder="Keywords..."
                className="w-full bg-white border border-gray-200 rounded-full py-2 pl-3 pr-9 text-xs focus:outline-none focus:border-violet-500 focus:ring-1 focus:ring-violet-500/20"
              />
          ...
          <div className="bg-gray-50 border border-gray-200 p-5 space-y-3">
            ...
            <div className="flex flex-col gap-1.5 text-xs text-gray-505 font-bold">
              <Link 
                href={`/software${getQueryString({ filter: 'all', page: '1' })}`} 
                className={`block px-2.5 py-1.5 rounded-none transition-all ${filter === 'all' ? 'bg-white text-violet-650 border-l-2 border-violet-650 shadow-sm' : 'hover:text-gray-800'}`}
              >
                All Tools
              </Link>
              <Link 
                href={`/software${getQueryString({ filter: 'free', page: '1' })}`} 
                className={`block px-2.5 py-1.5 rounded-none transition-all ${filter === 'free' ? 'bg-white text-emerald-600 border-l-2 border-emerald-600 shadow-sm' : 'hover:text-gray-800'}`}
              >
                Free Downloads
              </Link>
              <Link 
                href={`/software${getQueryString({ filter: 'premium', page: '1' })}`} 
                className={`block px-2.5 py-1.5 rounded-none transition-all ${filter === 'premium' ? 'bg-white text-violet-600 border-l-2 border-violet-600 shadow-sm' : 'hover:text-gray-800'}`}
              >
                Premium Software
              </Link>
            </div>
        ...
        <div className="lg:col-span-3 flex flex-col gap-8">
          ...
          {items.length === 0 ? (
            <div className="py-20 bg-gray-50 border border-gray-200 rounded-none flex flex-col items-center justify-center text-gray-400 p-8 text-center">
              ...
              <h3 className="text-base font-bold text-gray-700">No Software Found</h3>
              <p className="text-xs text-gray-400 max-w-sm mt-1">We couldn't find any software files matching your search criteria.</p>
            </div>
          ) : (
            <>
              ...
              {totalPages > 1 && (
                <div className="flex items-center justify-between border-t border-gray-150 pt-6 mt-4">
                  ...
                  <div className="flex items-center gap-2">
                    {page > 1 ? (
                      <Link
                        href={`/software${getQueryString({ page: (page - 1).toString() })}`}
