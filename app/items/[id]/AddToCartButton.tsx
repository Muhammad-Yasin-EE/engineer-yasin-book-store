...
export default function AddToCartButton({ item, buyNow }: AddToCartButtonProps) {
  const router = useRouter()
  const { addToCart, isInCart } = useCart()
  const [justAdded, setJustAdded] = useState(false)

  const isAlreadyInCart = isInCart(item.id)
  ...
  if (buyNow) {
    return (
      <button
        ...
        className="flex-grow inline-flex items-center justify-center gap-2 py-3 px-6 rounded-full bg-[#B8212E] hover:bg-[#D62636] text-white font-bold text-sm shadow-sm transition-all hover:-translate-y-0.5 cursor-pointer animate-scale-in"
      >
