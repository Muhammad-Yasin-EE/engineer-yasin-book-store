...
export async function POST(request: Request) {
  try {
    ...
    if (action === 'verify') {
      ...
      // 5. Fetch order items (books purchased)
      ...
      // 6. Insert purchases linking user and books
      ...
      if (purchaseErr) {
        // Handle unique constraint conflict (user already owns the book)
      ...
      if (resend && buyerEmail) {
        const bookLinks = items
          .map((item: any) => {
            const title = Array.isArray(item.books) ? item.books[0]?.title : item.books?.title;
            return `<li><strong>${title || 'Premium Ebook'}</strong>: <a href="${appUrl}/api/download?bookId=${item.book_id}" style="color: #4f46e5; font-weight: bold; text-decoration: underline;">Download Secure Link</a></li>`;
          })
          .join('')

        try {
          await resend.emails.send({
            from: 'Engineer Yasin Books <books@engineeryasin.com>',
            to: [buyerEmail],
            subject: 'Your Engineer Yasin Books order is confirmed',
            html: `
              <div style="font-family: sans-serif; max-width: 600px; margin: 0 auto; padding: 20px; border: 1px solid #e2e8f0; border-radius: 12px; background-color: #ffffff; color: #1e293b;">
                <h2 style="color: #4f46e5; border-bottom: 2px solid #e2e8f0; padding-bottom: 10px; margin-top: 0;">Engineer Yasin Books</h2>
                <p>Dear ${buyerName},</p>
                ...
                <p>The premium resources listed below are now permanently available in your library dashboard. Click the links below to download them securely:</p>
                <ul style="padding-left: 20px; line-height: 1.6;">
                  ${bookLinks}
                </ul>
                <p style="margin-top: 25px;">You can also access your downloads at any time by logging into your account at <a href="${appUrl}/account" style="color: #4f46e5;">engineeryasin.com/account</a>.</p>
                <hr style="border: 0; border-top: 1px solid #e2e8f0; margin: 20px 0;" />
                <p style="font-size: 11px; color: #64748b; text-align: center;">For assistance, please reply to this email or contact support@engineeryasin.com.</p>
              </div>
            `,
          })
        } catch (resendError) {
    ...
    if (action === 'reject') {
      ...
      if (resend && buyerEmail) {
        const reasonStr = rejectionReason || 'The transaction reference number or receipt screenshot could not be matched with our accounts.'
        
        try {
          await resend.emails.send({
            from: 'Engineer Yasin Books <books@engineeryasin.com>',
            to: [buyerEmail],
            subject: 'Update regarding your Engineer Yasin Books order',
            html: `
              <div style="font-family: sans-serif; max-width: 600px; margin: 0 auto; padding: 20px; border: 1px solid #e2e8f0; border-radius: 12px; background-color: #ffffff; color: #1e293b;">
                <h2 style="color: #ef4444; border-bottom: 2px solid #e2e8f0; padding-bottom: 10px; margin-top: 0;">Engineer Yasin Books</h2>
                <p>Dear ${buyerName},</p>
                ...
                <p>If you believe this is a mistake, or if you need to submit correct transaction details, please re-submit your receipt or contact our support team at <strong style="color: #4f46e5;">support@engineeryasin.com</strong> with your transaction ID and order number.</p>
                <hr style="border: 0; border-top: 1px solid #e2e8f0; margin: 20px 0;" />
                <p style="font-size: 11px; color: #64748b; text-align: center;">This is an automated notification. For assistance, contact support@engineeryasin.com.</p>
              </div>
            `,
          })
        } catch (resendError) {
