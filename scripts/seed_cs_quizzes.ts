// seed_cs_quizzes.ts
// Reads computer_science.json and creates quizzes in Supabase (30 Q per quiz, 15 min timer)
...
const QUIZ_SIZE = 30;
const CATEGORY_ID = 'nts'; // Category for NTS (Computer Science)

async function main() {
  const jsonPath = path.resolve('output', 'computer_science.json');
  if (!fs.existsSync(jsonPath)) {
    console.error('Computer Science JSON not found at', jsonPath);
    return;
  }
  ...
  for (let i = 0; i < numQuizzes; i++) {
    ...
    const { data: quiz, error: quizErr } = await supabase.from('quizzes').insert({
      title: `Computer Science Quiz ${i + 1}`,
      description: `Computer Science practice test with ${slice.length} questions.`,
    ...
    else console.log(`Created Computer Science Quiz ${i + 1} with ${slice.length} Q`);
  }
}
